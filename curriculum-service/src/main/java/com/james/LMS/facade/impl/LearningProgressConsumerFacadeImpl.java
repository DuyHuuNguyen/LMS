...
public class LearningProgressConsumerFacadeImpl implements LearningProgressConsumerFacade {
  ...
  private final LearningProgressService learningProgressService;
     ...
     * <p>Business flow:
     * <ol>
     *     <li>Find the existing {@link LearningProgress} of the user in the curriculum.</li>
     *     <li>If no progress exists, create a new one with the received learning minutes.</li>
     *     <li>Verify that the video in the message actually belongs to the user's curriculum.</li>
     *     <li>If this is a newly created progress, associate it with the corresponding
     *         {@link UserCurriculum}.</li>
     *     <li>If the message references a video outside of the curriculum, or references
     *         a different {@link UserCurriculum} than the existing progress, reject the
     *         message to prevent invalid updates.</li>
     *     <li>Accumulate the learning time and persist the updated progress.</li>
     * </ol>
     *
     ...
     *
     * @param message RabbitMQ message containing user id, curriculum id, video id,
     *                learning minutes and request tracing information.
     */
  @Transactional
  ...
  public void handleVideo(LearningProgressMessage message) {
     ...
     *
     * <p>This method follows the same validation strategy as {@code handleVideo()},
     * but verifies the relationship using the exam identifier instead of the video
     * identifier.
     *
     * <p>Business flow:
     * <ol>
     *     <li>Load or create the user's {@link LearningProgress} for the curriculum.</li>
     *     <li>Verify that the exam belongs to the user's curriculum.</li>
     *     <li>Associate the progress with the {@link UserCurriculum} if it is newly created.</li>
     *     <li>Reject the message if the exam does not belong to the curriculum or the
     *         existing progress references another curriculum.</li>
     *     <li>Add the received learning minutes and save the updated progress.</li>
     * </ol>
     *
     * <p>This validation guarantees that asynchronous exam events cannot update
     * learning progress belonging to another curriculum.
     *
     * @param message RabbitMQ message containing user id, curriculum id, exam id,
     *                learning minutes and request tracing information.
     */
  @Transactional
